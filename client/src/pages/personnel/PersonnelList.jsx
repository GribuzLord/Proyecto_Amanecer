...
// Coincide con la semilla de server/database/schema.sql
const HABILITACIONES = [
  'presidente', 'consejero_auxiliar', 'tesoro_1', 'perlas_escondidas', 'lectura_biblia',
  'conversaciones_1', 'conversaciones_2', 'discurso_estudiante', 'vida_cristiana_tema',
  'estudio_congregacion', 'lector_estudio', 'oracion_final',
  'presidente_atalaya', 'conductor_atalaya', 'lector_atalaya', 'oracion_final_atalaya',
];

const vacio = { nombre: '', genero: 'M', privilegio: 'publicador_bautizado', habilitaciones: [] };
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={guardar} className="bg-white border border-slate-200 rounded-2xl p-6 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, privilegio: e.target.value })}
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm"
              >
                {PRIVILEGIOS.map((p) => (
          ...
          <label className="block text-sm font-medium text-slate-600 mb-2">Partes que puede realizar</label>
            ...
            {HABILITACIONES.map((h) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                  form.habilitaciones.includes(h)
            ...
            <tbody className="divide-y divide-slate-100">
              {personas.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="px-4 py-2.5 text-slate-500">{p.genero}</td>
                  <td className="px-4 py-2.5 text-slate-500">{p.privilegio.replace(/_/g, ' ')}</td>
                  <td className="px-4 py-2.5 text-slate-500">{p.habilitaciones.length}</td>
                  <td className="px-4 py-2.5 text-right">
                    <button onClick={() => eliminar(p.id)} className="text-red-500 hover:underline text-xs">
                      Eliminar
                    </button>
                  </td>
                </tr>
